post_stage4a_distance_band_spec_v2 <- function() {
...
post_stage4a_distance_band_terms_v2 <- function() {
...
post_stage4a_classify_distance_band_links_v2 <- function(links) {
  ...
  bands <- post_stage4a_distance_band_spec_v2()
...
post_stage4a_add_distance_band_exposure_v2 <- function(events, links) {
  ...
  classified <- post_stage4a_classify_distance_band_links_v2(selected_links)
  ...
  band_lookup <- post_stage4a_distance_band_spec_v2()
...
post_stage4a_distance_band_formula_v2 <- function(response) {
  fixed <- c(
...
post_stage4a_distance_band_contrasts_v2 <- function(coefficient_names) {
  bands <- post_stage4a_distance_band_spec_v2()
...
post_stage4a_fit_distance_band_component_v2 <- function(
  ...
  formula <- post_stage4a_distance_band_formula_v2(response)
  fit <- try(
    if (outcome == "detection") {
      lme4::glmer(
  ...
  definitions <- post_stage4a_distance_band_contrasts_v2(names(beta))
  ...
  effects <- do.call(rbind, lapply(definitions, function(definition) {
    ...
    data.frame(
      model_version_id = "SOG_BALD_EAGLE_DISTANCE_BAND_v2",
...
post_stage4a_plot_bald_eagle_distance_bands_v2 <- function(
    effects, output_path) {
  plotted <- effects[
  ...
  mtext(
    "Bald Eagle response timing by source-point distance, 0–26 km",
...
run_post_stage4a_distance_band_sensitivity_v2 <- function(
    execution_code_commit,
    output_dir = "outputs/post_stage4a_distance_band_sensitivity_v2") {
  acknowledgement <- "through_2025_bald_eagle_distance_bands_26km_v2"
  if (!identical(
      Sys.getenv("POST_STAGE4A_DISTANCE_BAND_SENSITIVITY_V2_AUTHORIZED"),
      acknowledgement
  )) {
    stop(
  ...
  code_signature <- paste(
    ...
    protected_hashes,
    .post_stage4a_sha256_v1(
      "metadata/post_stage4a_distance_band_sensitivity_spec_v2.yml"
    ),
    sep = "|",
    collapse = "|"
  )
  ...
  .post_stage4a_write_csv_v1(
    effects, file.path(output_dir, "bald_eagle_distance_band_effects_v2.csv")
  )
  .post_stage4a_write_csv_v1(
    diagnostics, file.path(output_dir, "model_diagnostics_v2.csv")
  )
  .post_stage4a_write_csv_v1(
    term_support, file.path(output_dir, "model_term_support_v2.csv")
  )
  .post_stage4a_write_csv_v1(
    exposure_support, file.path(output_dir, "joint_exposure_support_v2.csv")
  )
  .post_stage4a_write_csv_v1(
    fixed_effects, file.path(output_dir, "fixed_effects_v2.csv")
  )
  .post_stage4a_write_csv_v1(
    exposure_covariance,
    file.path(output_dir, "exposure_covariance_v2.csv")
  )
  figure_path <- file.path(
    output_dir, "bald_eagle_distance_band_panel_v2.png"
  )
  post_stage4a_plot_bald_eagle_distance_bands_v2(effects, figure_path)

  execution_record <- list(
    execution_version = "post_stage4a_distance_band_sensitivity_v2",
    ...
    authorization_record =
      "metadata/post_stage4a_distance_band_sensitivity_authorization_v2.yml",
    ...
    final_gate =
      "PASS_PENDING_HUMAN_BALD_EAGLE_26KM_DISTANCE_BAND_REVIEW"
  )
  yaml::write_yaml(
    execution_record,
    file.path(output_dir, "execution_record_v2.yml")
  )
  output_files <- file.path(
    output_dir,
    c(
      "bald_eagle_distance_band_effects_v2.csv",
      "model_diagnostics_v2.csv",
      "model_term_support_v2.csv",
      "joint_exposure_support_v2.csv",
      "fixed_effects_v2.csv",
      "exposure_covariance_v2.csv",
      "bald_eagle_distance_band_panel_v2.png",
  ...
  .post_stage4a_write_csv_v1(
    manifest, file.path(output_dir, "output_hash_manifest_v2.csv")
  )
  message(
