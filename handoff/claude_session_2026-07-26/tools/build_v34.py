...
OUT = "/sessions/keen-confident-allen/mnt/outputs/mer_manuscript_v34.docx"
...
# The case study pointed at three supplementary figures that do not exist in the
# supplement. Figure 5 is the placeholder for exactly that content, so point
# there instead of at phantom numbering.
EXTRA_EDITS = [
 ("The three species separate a localized waterbird response from broader "
  "event-time structure (Figs. S4 to S6).",
  "The three species separate a localized waterbird response from broader "
  "event-time structure (Figure 5)."),
]
...
for p in P:
    ...
    for old, new in EXTRA_EDITS:
        if old in t:
            ...
            n_extra += 1
assert n_extra == len(EXTRA_EDITS), "extra edit not matched"
...
json.dump(target, open("v34_target.json", "w"))
print("headings retitled : %d" % n_head)
print("ordination cut    : %d paragraph" % n_ord)
print("cross-refs fixed  : %d" % n_extra)
