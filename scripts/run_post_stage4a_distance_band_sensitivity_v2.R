...
if (mode == "fixture") {
  ...
  classified <- post_stage4a_classify_distance_band_links_v2(links)
  expected <- c(
    "band_0_2", "band_0_2",
  ...
  stopifnot(identical(
    post_stage4a_classify_distance_band_links_v2(boundary_20)$band,
  ...
  joint <- post_stage4a_add_distance_band_exposure_v2(
    events, paired_links
  )
  stopifnot(
    ...
    length(post_stage4a_distance_band_terms_v2()) == 78L
  )
  ...
  message("POST_STAGE4A_DISTANCE_BAND_SENSITIVITY_V2_FIXTURE=PASS")
  quit(status = 0L)
}

code_files <- c(
  "R/post_stage4a_distance_band_sensitivity_v2.R",
  "scripts/run_post_stage4a_distance_band_sensitivity_v2.R",
  "scripts/run_post_stage4a_distance_band_sensitivity_v2.ps1",
  "scripts/postprocess_bald_eagle_distance_band_sensitivity_v2.R",
  "scripts/build_post_stage4a_distance_26km_links_v2.ps1",
  "scripts/preflight_post_stage4a_distance_bands_v2.R",
  ...
  "tests/testthat/test-post-stage4a-distance-band-sensitivity-v2.R"
)
dirty <- system2(
  "git",
...
if (length(dirty) && any(nzchar(dirty))) {
  stop(
    "Production is blocked until the v2 code and specification are committed",
    call. = FALSE
  )
}
...
if (length(execution_code_commit) != 1L ||
  ...
  stop("Unable to resolve committed v2 execution code", call. = FALSE)
}
run_post_stage4a_distance_band_sensitivity_v2(
