...
$runner = Join-Path $PSScriptRoot 'run_post_stage4a_distance_band_sensitivity_v1.R'

if (-not (Get-Command Rscript -ErrorAction SilentlyContinue)) {
    throw 'Rscript is not available on PATH.'
}
if ($Mode -eq 'production' -and
    $env:POST_STAGE4A_DISTANCE_BAND_SENSITIVITY_AUTHORIZED -ne
        'through_2025_bald_eagle_distance_bands_v1') {
    throw @'
Production requires:
$env:POST_STAGE4A_DISTANCE_BAND_SENSITIVITY_AUTHORIZED =
  "through_2025_bald_eagle_distance_bands_v1"
'@
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Distance-band runner exited with code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
