...
if (!identical(target_id, "atx_725074cbfcb6")) {
  stop("PREFLIGHT_TAXON_GATE: Glaucous-winged Gull mismatch", call. = FALSE)
}
...
aggregate_dir <- file.path(
  "outputs", "post_stage4a_gwgu_distance_band_sensitivity_v1_preflight"
)
dir.create(aggregate_dir, recursive = TRUE, showWarnings = FALSE)
utils::write.csv(
  support,
  file.path(aggregate_dir, "candidate_2km_model_term_support_v1.csv"),
...
utils::write.csv(
  summary,
  file.path(aggregate_dir, "candidate_2km_band_minimum_support_v1.csv"),
  row.names = FALSE, quote = TRUE, na = ""
)
record <- list(
...
yaml::write_yaml(
  record,
  file.path(aggregate_dir, "preflight_execution_record_v1.yml")
)
message("POST_STAGE4A_GWGU_DISTANCE_BAND_V1_PREFLIGHT=PASS")
