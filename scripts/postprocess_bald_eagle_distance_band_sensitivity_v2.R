...
code_files <- c(
  "R/post_stage4a_distance_band_sensitivity_v2.R",
  "scripts/postprocess_bald_eagle_distance_band_sensitivity_v2.R"
)
dirty <- system2(
  "git",
...
effects_path <- file.path(
  output_dir, "bald_eagle_distance_band_effects_v2.csv"
)
fixed_path <- file.path(output_dir, "fixed_effects_v2.csv")
covariance_path <- file.path(output_dir, "exposure_covariance_v2.csv")
record_path <- file.path(output_dir, "execution_record_v2.yml")
stopifnot(
...
if (!setequal(unique(effects$outcome), outcomes) ||
  ...
  stop("POSTPROCESS_OUTCOME_GATE: expected two Bald Eagle outcomes",
       call. = FALSE)
}
...
.post_stage4a_write_csv_v1(
  tests, file.path(output_dir, "distance_heterogeneity_tests_v2.csv")
)

post_stage4a_plot_bald_eagle_distance_bands_v2(
  effects,
  file.path(output_dir, "bald_eagle_distance_band_panel_v2.png")
)
...
output_files <- file.path(
  ...
  c(
    ...
    "distance_heterogeneity_tests_v2.csv",
...
message("BALD_EAGLE_DISTANCE_BAND_POSTPROCESS_V2=PASS")
