#!/usr/bin/env python3
"""Build manuscript-safe v7 tables and figures from completed event-study outputs.
...
Changes from the v6 builder, all made in response to editorial review of the v6
draft:

* the active-period forest plot now marks BH q < 0.05, which its v6 caption
  claimed but the v6 figure never drew;
* the timing figure now uses ``did_pre_14_day``, matching the pre-spawn summary
  defined in the Methods and reported in the Results text, instead of the
  ``did_pre_7_day`` window the v6 figure actually plotted under a caption that
  named the 14-day summary;
* a complete 49-species figure (Figure 2) replaces the implicit claim that the 11-species
  panel represents the family;
* the two panels of each forest plot share one x-range so that the
  detection/flock-size comparison is visual rather than arithmetic;
* every figure is drawn in Times New Roman at 500 dpi; and
* the duplicated ``Table_4_*`` outputs are replaced by a single numbered series.
"""
...
def write_tables(
    ...
    active.to_csv(tables_dir / "Table_1_main_species_active_v7.csv", index=False)
    ...
    extremes["outcome"] = extremes["outcome"].map(OUTCOME_LABELS)
    ...
    extremes.to_csv(tables_dir / "Table_2_family_extremes_v7.csv", index=False)

    effects[effects["analysis_role"].eq("core_species")].to_csv(
        tables_dir / "Table_S1_event_study_all_49_species_v7.csv", index=False
    )
    periods = effects[
    ...
    periods.to_csv(
        tables_dir / "Table_S2_main_species_periods_v7.csv", index=False
    )
    effects[effects["analysis_role"].eq("specificity_comparator")].to_csv(
        tables_dir / "Table_S3_specificity_comparators_v7.csv", index=False
    )
...
def write_handoff(effects: pd.DataFrame, generated_dir: Path) -> None:
    ...
    for outcome, label in OUTCOME_LABELS.items():
        ...
        lines.append("")
    (generated_dir / "event_study_results_handoff_v7.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def plot_active(effects: pd.DataFrame, figures_dir: Path) -> None:
    ...
    fig.savefig(
        figures_dir / "Figure_1_sog_active_event_study_v7.png",
        dpi=FIGURE_DPI,
        bbox_inches="tight",
        facecolor="white",
    )
...
def plot_timing(effects: pd.DataFrame, figures_dir: Path) -> None:
    ...
    colorbar.set_label("Log ratio of near/reference ratios", fontsize=9, labelpad=7)
    fig.suptitle(
    ...
    fig.text(
        0.5,
        0.015,
        "Cells show exponentiated difference-in-differences relative to the "
        "-28 to -15 day baseline; * denotes BH q < 0.05.",
        ha="center",
        fontsize=9,
    )
    fig.subplots_adjust(left=0.19, right=0.84, bottom=0.11, top=0.91, wspace=0.08)
    fig.savefig(
        figures_dir / "Figure_3_sog_event_timing_v7.png",
        dpi=FIGURE_DPI,
        bbox_inches="tight",
        pad_inches=0.15,
        facecolor="white",
...
def plot_all_species(effects: pd.DataFrame, figures_dir: Path) -> None:
    ...
    fig.savefig(
        figures_dir / "Figure_2_sog_all_species_v7.png",
        dpi=FIGURE_DPI,
        bbox_inches="tight",
        facecolor="white",
    )
...
def main() -> int:
    ...
    tables_dir = args.manuscript_dir / "tables_v7"
    figures_dir = args.manuscript_dir / "figures_v7"
    generated_dir = args.manuscript_dir / "generated_v7"
    ...
    print("MER_V7_EVENT_STUDY_ASSETS=PASS")
    return 0
