...
$runner = Join-Path $PSScriptRoot `
    'run_post_stage4a_descriptive_summaries_v1.R'
...
if ($Mode -eq 'production' -and
    [string]::IsNullOrWhiteSpace(
        $env:HERRING_EBIRD_V2_HERRING
    )) {
    throw @'
Production requires HERRING_EBIRD_V2_HERRING to point to the frozen source
whose SHA-256 is registered by the descriptive runner.
'@
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Descriptive runner exited with code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
